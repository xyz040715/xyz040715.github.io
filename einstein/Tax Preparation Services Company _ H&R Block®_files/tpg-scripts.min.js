...
/* tpg-pricegroup.js */
function setTpgGroup(name){var pgVal="";var _now=new Date;var _expires="Thu, Apr 19 2019 05:00:00 UTC";_expires=new Date(_expires);if(_expires-_now<0){HRB.permanent.storeValue("tpgGroup",null);return null}if(typeof localStorage["tpgGroup"]==="undefined"){var nameCK=name+"=";var cookieList=document.cookie.split(";");for(var i=0;i<cookieList.length;i++){var c=cookieList[i];while(c.charAt(0)==" ")c=c.substring(1,c.length);if(c.indexOf(nameCK)==0)pgVal=c.substring(nameCK.length,c.length)}if(pgVal&&(pgVal===
"A"||pgVal==="B")){HRB.permanent.storeValue("tpgGroup",pgVal);localStorage.setItem("tpgGroupCreated",_now)}}else pgVal=localStorage.tpgGroup;if(!pgVal||!(pgVal==="A"||pgVal==="B")){pgVal=Math.random()<.5?"A":"B";HRB.permanent.storeValue("tpgGroup",pgVal);localStorage.setItem("tpgGroupCreated",_now)}return localStorage.tpgGroup};
