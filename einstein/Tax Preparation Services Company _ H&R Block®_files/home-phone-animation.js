function EZSlots(id,useroptions){
	...
	this.time = options.time ? (options.time * 500) : 1000; //time in millis for a spin to take
	...
	this.spinOne = function(jqoSlider,whichReel,shouldWin){
		...
		jqoSlider.stop(true,true).animate(
			...
			//----------- ADJUST SPEED AND EASING HERE     SPEED/EASE-ADJUSTMENT
			{'duration' : that.time + Math.round(Math.random()*200), 'easing' : "easeOutQuad"});
			//{'duration' : that.time + Math.round(Math.random()*1000), 'easing' : "easeOutSine"});
			//{'duration' : that.time + Math.round(Math.random()*1000), 'easing' : "easeOutExpo"});
		return chosen;
	}
