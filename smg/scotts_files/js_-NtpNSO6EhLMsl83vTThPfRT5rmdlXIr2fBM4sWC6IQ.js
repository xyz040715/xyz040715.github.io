...
/**
 * @file
 * Supporting code for the Scotts D8 platform DTM integration.
 */


/**
 * Initializes the DTM object which is required by DTM.
 */
if (typeof dtmData == 'undefined'){
  dtmData = {};
}

// Overriding dtmData cms.
dtmData.cms = 'D';
...
for(var i = 0; i < count; i++) {
  links[i].addEventListener('click', function(e) {
    if (e.target.id === 'facebook' || e.target.id === 'twitter' || e.target.id === 'googleplus') {
      dtmData.events.push({
          name: 'socialClick',
...
var TagID_product_name = "product_name";
...
var TagID_article_name = "article_name";
...
var TagID_purchase_online = "purchase_online";
var TagID_authored_date = "authored_date";
var TagID_last_saved_date = "last_saved_date";
var TagID_star_rating_avg = "star_rating_avg";
var TagID_review_count = "review_count";

// DTM specific names.
var DTM_product_id = "product_id";
var DTM_product_name = "product_name";
...
var DTM_article_name = "article_name";
...
var DTM_purchase_online = "purchase_online";
var DTM_authored_date = "authored_date";
var DTM_last_saved_date = "last_saved_date";
var DTM_star_rating_avg = "star_rating_avg";
var DTM_review_count = "review_count";
...
/**
 * If the function being called needs to have multiple arguments, pass the arguments
 * as an array and set isMultipleArgsArray to true.
 */
...
function SetTrackingTag(name, value) {
  if (value != null && typeof value === 'object' && value["#markup"]) {
    dtmData[window["DTM_" + name]] = value["#markup"];
  } else {
...
/**
 * @file
 * Drupal data connector for Scotts Analytics.
 *
 * Look for analytics lists in drupalSettings and use TrackingTag
 * functions to insert into analytics pick-up location (dtmData, etc.)
 * NOTE: important that this be SYNCRONOUS js and not on page-ready
 * because some tag-mgrs (Adobe DTM) insist their page collector be
 * called SYNCRONOUSLY at end of page body.
 */

if (drupalSettings) {
    if (drupalSettings.scotts_analytics) {

        // SetTrackingTag calls.
        if (drupalSettings.scotts_analytics.setTags) {
            for (var idx in drupalSettings.scotts_analytics.setTags) {
                var tagCall = drupalSettings.scotts_analytics.setTags[idx];
                SetTrackingTag(window[tagCall.id], tagCall.value);
            }
        }

        // AppendTrackingTag calls.
        if (drupalSettings.scotts_analytics.appendTags) {
            for (var idx in drupalSettings.scotts_analytics.appendTags) {
                var tagCall = drupalSettings.scotts_analytics.appendTags[idx];
                AppendTrackingTag(window[tagCall.id], tagCall.value);
            }
        }

        // Apply preferred locale from the client.
        SetTrackingTag(TagID_preferred_locale, ScottsLocaleAnalytics.getBrowserLanguage());
    }
}
;
if (typeof _satellite !== 'undefined') {
  (function(tracker, $) {
      ...
      client: (function() {
        ...
        Client.prototype = {
          ...
          push: function(key, value) {
            ...
            this.log("Sending message [MESSAGE]".replace(/MESSAGE/, message));
        ...
        function Client() {
          this.log("initializing new AnalyticsPipe.Client");
        }
      ...
      server: (function() {
        ...
        Server.prototype = {
          log: function(message) {
            ...
            if (window.location.hash.indexOf("analytics-pipe") === -1) {
              return;
            }


            message = ['PIPE SERVER: ', message].join('');
            console.log(message);
            $(window).trigger('analyticsPipeServerMessageLog', {
              message: message
            });
          ...
          logRaw: function(message) {
            console.log(message);
            $(window).trigger('analyticsPipeClientMessageLog', {
              message: message
            });
          },

          store: function(key, value) {
            if ("object" === typeof this.dataStore) {
              this.dataStore[key] = value;
              dtmData[key] = value;
              if ("object" === typeof this.tracker) {
                this.tracker.setVar(key, value);
              } else {
                this.log("I don't have Adobe Analytics.");
              }
            }
          },

          track: function(key) {
            this.log("I want Adobe to send this direct call message [MESSAGE]".replace(/MESSAGE/, key));
          ...
          listen: function() {
            ...
            var self = this;
            var productIdTagKeys = ["Product_Visited", "Product_Applied", "Product_Skipped"];
            var handleMessage = function(e) {
              var json, key, value;
              // Validating the event has a data payload, ignore if it hasn't
              try {
              ...
              try {
                if (json && 'DTM' === json.messageType) {
                  self.log("Received: " + e.data);
                  key = json.key;
                  value = json.value;

                  //If the value is to be recorded to the product id pass it to the dtmData object
                  if (productIdTagKeys.indexOf(key) != -1) {
                    self.log("Setting a product id tag with value: " + value);
                    self.store(key, value);
                    //Setting the key value pair in the dtmData object.
                    dtmData.product_id = value;
                  }
                  // If the value is to be recorded to the product_sku, product_revenue and passed it to the dtmData object.
                  else if ('product_added_to_cart' === json.key) {
                    self.log("Setting a product_added_to_cart event product_sku: " + json.product_sku);
                    self.store('product_sku', json.product_sku);
                  }
                  ...
                  if (json && 'clientMessage' === json.messageType) {
                    // message from client
                    self.logRaw(json.message);
                  }
                }
              } catch (err) {
        ...
        function Server(tracker, dataStore) {

          this.log("initializing new AnalyticsPipe.Server");
    ...
    var server = new AnalyticsPipe.server(tracker, {});
    server.listen();
